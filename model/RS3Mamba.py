...
import timm
import re
from model.LSKNet import LSKmodule
from model.SViT import StokenAttention
...
class FusionAttention(nn.Module):
    def __init__(self,
        ...
        # 深度卷积
        self.dconv5_5 = nn.Conv2d(ssmdims, ssmdims, kernel_size=5, padding=2, groups=ssmdims)
        self.dconv1_7 = nn.Conv2d(ssmdims, ssmdims, kernel_size=(1,7), padding=(0,3), groups=ssmdims)
        self.dconv7_1 = nn.Conv2d(ssmdims, ssmdims, kernel_size=(7,1), padding=(3,0), groups=ssmdims)
        self.dconv1_11 = nn.Conv2d(ssmdims, ssmdims, kernel_size=(1,11), padding=(0,5), groups=ssmdims)
        self.dconv11_1 = nn.Conv2d(ssmdims, ssmdims, kernel_size=(11,1), padding=(5,0), groups=ssmdims)
        self.dconv1_21 = nn.Conv2d(ssmdims, ssmdims, kernel_size=(1,21), padding=(0,10), groups=ssmdims)
        self.dconv21_1 = nn.Conv2d(ssmdims, ssmdims, kernel_size=(21,1), padding=(10,0), groups=ssmdims)
        self.conv_o = nn.Conv2d(ssmdims, ssmdims, kernel_size=(1,1), padding=0)
        self.act = nn.GELU()

        # DilateFormer
        # self.attn_dilateformer = MultiDilatelocalAttention(dim=dim)
        # LSKNet
        # self.lsknet = LSKmodule(dim=dim)
        # SViT
        self.svit = StokenAttention(dim=dim, stoken_size=[16, 16]).cuda()
    ...
    def forward(self, x, y):
        ...
        B, C, H, W = x.shape
        y = self.act(self.conv_o(y))
        y_init = self.dconv5_5(y)
        y_1 = self.dconv1_7(y_init)
        y_1 = self.dconv7_1(y_1)
        y_2 = self.dconv1_11(y_init)
        y_2 = self.dconv11_1(y_2)
        y_3 = self.dconv1_21(y_init)
        y_3 = self.dconv21_1(y_3)
        y = y_init + y_1 + y_2 + y_3
        spatial_a = self.conv_o(y)
        y = y_init * spatial_a
...
class FusionBlock(nn.Module):
    ...
    def forward(self, x, y):
        # Fm + Fa 的layernorm层归一化
        # 全局线性注意力机制FusionAttention
...
class RS3Mamba(nn.Module):
    ...
    def forward(self, x):
        h, w = x.size()[-2:]
        # 2D卷积和归一化操作（归一化中引用仿射参数）
        ssmx = self.stem(x)
        vss_outs = self.vssm_encoder(ssmx) # 48*128*128, 96*64*64, 192*32*32, 384*16*16, 768*8*8

        ress = []
        # swsl_resnet18模型的conv1, bn1, act1, maxpool层结构。
        # 结构与原模型中层名称相关， 同时使用预训练的权重。
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.act1(x)
        x = self.maxpool(x)
...
def load_pretrained_ckpt(
    model, 
    ckpt_path="/root/autodl-tmp/RS3Mamba/pretrained_ckpt/vssmtiny_dp01_ckpt_epoch_292.pth"
):
