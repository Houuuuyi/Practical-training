    ...
    def __init__(self, num_feat, is_light_sr= False, compress_ratio=3,squeeze_factor=4):
        ...
        if is_light_sr: # we use depth-wise conv for light-SR to achieve more efficient
            self.cab = nn.Sequential(
                ...
                ChannelAttention(num_feat, num_feat // squeeze_factor)
            )
        else: # for classic SR
            self.cab = nn.Sequential(
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # 深度卷积
        # self.DWconv = nn.Conv2d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=3, padding=2, groups=hidden_dim)
        # 注意力
        self.conv_blk = CAB(hidden_dim, is_light_sr=False)
    def forward(self, input: torch.Tensor):
        # input torch.Size([10, 64, 64, 96])
        x = input*self.skip_scale1 + self.drop_path(self.self_attention(self.ln_1(input)))
        # x torch.Size([10, 64, 64, 96])
...
class VSSMEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pos_drop(x)
        # self.layer代表了VSS block块
        # self.downsample--------------->patch embedding下采样，形状改变缩减为1/2， 通道数增加1倍
        for s, layer in enumerate(self.layers):
            # input x : torch.Size([10, 64, 64, 96])----torch.Size([10, 32, 32, 192])---torch.Size([10, 16, 16, 384])---torch.Size([10, 8, 8, 768])
            x = layer(x) #VSS_layer不改变原本的形状
            x_ret.append(x.permute(0, 3, 1, 2))
            # downsamples代表了patch embedding 处理少一次
